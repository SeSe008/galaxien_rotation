...
pub fn calculate_velocity(radius: f64, density_disk: f64, scale_disk: f64, density_halo: f64, scale_halo: f64, iso_nfw: bool) -> f64 {
    ...
    let v = (calculate_velocity_disk(radius, density_disk, scale_disk).powi(2)+ calculate_velocity_halo(radius, density_halo, scale_halo, iso_nfw).powi(2)).sqrt();
    v * (kpc_to_m()).sqrt() * m_s_to_km_s()
}
...
fn calculate_velocity_disk(radius: f64, density_disk: f64, scale_disk: f64) -> f64 {
    ...
    (4.0 * PI * G * density_disk * scale_disk * gamma.powi(2) * (besseli(gamma, 0) * besselk(gamma, 0) - besseli(gamma, 1) * besselk(gamma, 1))).sqrt()
}

//Calculate the velocity of the halo
fn calculate_velocity_halo(radius: f64, density_halo: f64, scale_halo: f64, iso_nfw: bool) -> f64 {
    (G * mass_halo(radius, density_halo, scale_halo, iso_nfw) / radius).sqrt()
}